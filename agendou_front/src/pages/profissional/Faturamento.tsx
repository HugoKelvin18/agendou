...
export default function Faturamento() {
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-green-50 to-emerald-100">
                <div className="text-center">
                    ...
                    <p className="text-base md:text-lg text-gray-700">Carregando dados de faturamento...</p>
                </div>
            </div>
        );
    }
    ...
    if (!data || !data.resumo) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-green-50 to-emerald-100">
                <div className="text-center">
                    <p className="text-base md:text-lg text-gray-700">Nenhum dado disponível</p>
                    <button
                        onClick={() => window.location.reload()}
                        className="mt-4 px-3 py-1.5 text-xs md:px-4 md:py-2 md:text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium"
                    >
                        Recarregar página
                    </button>
                </div>
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                            ...
                            <button
                                onClick={() => carregarFaturamento(periodo)}
                                disabled={loading}
                                className="flex items-center gap-2 px-3 py-1.5 text-xs md:px-4 md:py-2 md:text-sm bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed font-medium"
                            ...
                            <button
                                ...
                                disabled={!temDados}
                                className={`flex items-center gap-2 px-3 py-1.5 text-xs md:px-4 md:py-2 md:text-sm rounded-lg transition-all shadow-lg font-medium ${
                                    temDados 
                ...
                {!temDados && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 md:p-6 text-center">
                        ...
                        <button
                            onClick={() => carregarFaturamento(periodo)}
                            disabled={loading}
                            className="mt-4 flex items-center gap-2 px-3 py-1.5 text-xs md:px-4 md:py-2 md:text-sm bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-all mx-auto disabled:opacity-50 font-medium"
                        >
                            <RefreshCw size={18} className={loading ? "animate-spin" : ""} />
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                                <p className="text-xs md:text-sm text-blue-600 font-medium mb-1">Tipos de Serviços</p>
                                <p className="text-xl md:text-2xl font-bold text-blue-800">{porServico.length}</p>
                            </div>
                            <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                                <p className="text-xs md:text-sm text-green-600 font-medium mb-1">Período Analisado</p>
                                <p className="text-base md:text-lg font-bold text-green-800 capitalize">{resumo.periodo}</p>
                            </div>
                            <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                                <p className="text-xs md:text-sm text-purple-600 font-medium mb-1">Ticket Médio</p>
